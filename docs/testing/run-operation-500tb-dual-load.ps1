...
$root = Join-Path $repo "docs/testing/evidence/operation-500tb-dual-load"
$evidence = Join-Path $root $stamp
New-Item -ItemType Directory -Force -Path $evidence | Out-Null

$env:FORGETDM_LOAD_PG_URL = $PostgresUrl
$env:FORGETDM_LOAD_PG_USER = $PostgresUser
$env:FORGETDM_LOAD_PG_PASS = $PostgresPassword
$env:FORGETDM_LOAD_ORACLE_URL = $OracleUrl
$env:FORGETDM_LOAD_ORACLE_USER = $OracleUser
$env:FORGETDM_LOAD_ORACLE_PASS = $OraclePassword
$env:FORGETDM_LOAD_SAMPLE_ROWS = "$SampleRows"
$env:FORGETDM_ORACLE_SQLLOADER_ENABLED = "true"
$env:FORGETDM_ORACLE_SQLLOADER_BIN = $OracleSqlLoader
$env:FORGETDM_ORACLE_SQLLOADER_CONNECT = $OracleConnect
$env:FORGETDM_ORACLE_SQLLOADER_AUTH = "PASSWORD_STDIN"
$env:FORGETDM_NATIVE_LOAD_EVIDENCE_DIR = (Join-Path $evidence "oracle-native-evidence")

Push-Location $repo
try {
    & $maven '-Dtest=io.forgetdm.provision.loader.Operation500TbDualEngineLoadQualificationTest,io.forgetdm.provision.loader.OracleSqlLoaderExecutorTest' "-Dforgetdm.load.evidence.dir=$evidence" test
    if ($LASTEXITCODE -ne 0) { throw "Dual-engine load qualification failed ($LASTEXITCODE)" }
    Copy-Item (Join-Path $evidence "operation-500tb-dual-load.md") (Join-Path $root "latest-report.md") -Force
    Copy-Item (Join-Path $evidence "operation-500tb-dual-load.json") (Join-Path $root "latest-report.json") -Force
    Write-Host "Dual-engine load evidence: $evidence"
